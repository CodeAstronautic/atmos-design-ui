...
const ContactInfo: React.FC<ContactInfoProps> = ({ description, phone, email }) => {
  return (
    <div className="flex flex-col grow items-start max-md:mt-0">
      ...
      <div className="mt-[18px] text-[#5E626F] font-black text-[13px] max-md:mt-[18px]">{phone}</div>
      <div className="mt-2 text-[#5E626F] font-black text-[13px] leading-[19.5px]">{email}</div>
    </div>
  );
};
